...
CHANNEL_ID = '781388235678941204'  # Replace with your actual channel ID
...
DOWNLOAD_PATH = os.path.join(os.getcwd(), "Localvd")
...
def download_video(url, filename):
    """Downloads the video from the URL to the specified folder."""
    ...
    while True:
        ...
        for message in messages:
            for attachment in message.get("attachments", []):
                if any(attachment["filename"].endswith(ext) for ext in ['.mp4', '.webm', '.mov']):
                    download_video(attachment["url"], attachment["filename"])
